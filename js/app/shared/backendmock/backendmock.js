const backendMock = function(app) {
  app.run(function($httpBackend) {
    ...
    response.Comments.push({
      ProviderId: 2,
      ReceiverId: 4,
      Comment: "Good Support for Production Tickets",
      ProviderName: "Sabuj",
      ReceiverName: "Eric"
    });
    response.Comments.push({
      ProviderId: 3,
      ReceiverId: 6,
      Comment: "Good job in application devlopment",
      ProviderName: "James",
      ReceiverName: "Khalid"
    });
    response.Comments.push({
      ProviderId: 1,
      ReceiverId: 8,
      Comment: "Great commitement to tasks",
      ProviderName: "Jason",
      ReceiverName: "Deepa"
    });
    ...
    $httpBackend
      .whenGET(apiUrl + "GetInitialData")
      .respond(function(method, url, data) {
        return [200, response, {}];
      });

    $httpBackend
      .whenPOST(apiUrl + "UpdateRewardInfo")
      .respond(function(method, url, data) {
    ...
    var updateUserReward = function(modalData) {
      ...
      var modifiedUserIndex = _.findIndex(response.Users, {
        Id: rewardedUserId
      });
    ...
    $httpBackend
      .whenPOST(apiUrl + "UpdateRedeemInfo")
      .respond(function(method, url, data) {
        var modalData = JSON.parse(data);
